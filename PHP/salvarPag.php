...
/*REALIZAR A CONEXÃO COM O BANCO, PEGAR OS DADOS EXISTENTES E VERIFICAR
SE O DONO DO CPF DIGITADO JÁ ESTÁ CADASTRADO*/
$result = $conn->query($query_sits);
    ...
    if (($result) and ($result->num_rows != 0) ) {
        ...
        for ($i = 0; $i < count($dados); $i++) {
            if(($dados[$i]['cliente'] == $Nome) && ($dados[$i]['vencimento'] == $Vencimento)){
                ...
                $dadoExiste = true;
                echo '<script> alert("Este pagamento ja foi cadastrado"); const win = window.open("../HTML/principal.html","_self"); </script>';
            }
        }
    ...
    //SE FOR PARA REGISTRAR O PRIMEIRO USUÁRIO
        ...
        $query_sits = "INSERT INTO pagamentos(cliente, vencimento, pagamento, valor, forma, funcionario)  VALUES('$cliente', '$vencimento', '$pagamento', '$valor', '$forma', '$funcionario')"; 
